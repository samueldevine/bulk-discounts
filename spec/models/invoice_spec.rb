...
RSpec.describe Invoice, type: :model do
  ...
  describe '#total_revenue' do
    it 'returns the total revenue for a given merchant on an invoice' do
      ...
      invoice1 = Invoice.create!(customer_id: customer1.id, status: 'in progress')
      invoice2 = Invoice.create!(customer_id: customer1.id, status: 'completed')
      item1 = Item.create!(name: 'book', description: 'good book', unit_price: 1200, merchant_id: merchant1.id)
      item2 = Item.create!(name: 'spatula', description: 'good spatula', unit_price: 800, merchant_id: merchant1.id)
      invoice_item1 = InvoiceItem.create!(item_id: item1.id, invoice_id: invoice1.id, quantity: 2, unit_price: 2400, status: 'pending')
      invoice_item2 = InvoiceItem.create!(item_id: item2.id, invoice_id: invoice2.id, quantity: 2, unit_price: 1600, status: 'shipped')

      expect(invoice1.total_revenue(merchant1.id)).to eq(48)
      expect(invoice2.total_revenue(merchant1.id)).to eq(32)
    end
  end
end
