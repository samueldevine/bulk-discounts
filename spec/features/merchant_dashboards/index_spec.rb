...
RSpec.describe "Merchant's dashboard", type: :feature do
  ...
  describe 'displays' do
    ...
    it "a link to the merchant items index" do
      click_link "My Items"
      expect(current_path).to eq("/merchants/#{@merchant.id}/items")
    end

    it "a link to the merchant invoices index" do
      click_link "My Invoices"
      expect(current_path).to eq("/merchants/#{@merchant.id}/invoices")
    end

    it "a link to the bulk discounts index" do
      click_link "My Discounts"
      expect(current_path).to eq merchant_bulk_discounts_path(@merchant)
    end

    it "the 5 customer names with the most successful transactions" do
      visit "/merchants/#{@merchant.id}/dashboard"
    ...
    it 'the number of succesful transactions for the top 5 customers' do
    ...
    it 'a section for "Items Ready to Ship"' do
      within ("div#items-ready-to-ship") do
        expect(page).to have_content("Items Ready to Ship")
      end
    end

    it "each invoice id as a link to my merchant's invoice show page" do
      within ("div#items-ready-to-ship") do
    ...
    it "the date that each invoice was created, formatted 'Monday, July 18, 2019'" do
      date = @invoice_6.created_at.strftime("%A, %B %-d, %Y")
      expect(page).to have_content(date)
    end

    it "the item list ordered from oldest to newest" do
      within ("div#items-ready-to-ship") do
